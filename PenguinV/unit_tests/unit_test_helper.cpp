...
namespace
{
    size_t randomWidth()
    {
        return Unit_Test::randomValue<size_t>( 1, 2048 );
    };
...
namespace Unit_Test
{
    ...
    std::vector < data > generateArray( size_t size, size_t maximumValue )
    {
    ...
    {
        width  = randomValue<size_t>( 1, image.width()  + 1 );
        height = randomValue<size_t>( 1, image.height() + 1 );

        x = randomValue<size_t>( image.width()  - width );
        y = randomValue<size_t>( image.height() - height );
    }

    void generateRoi( const std::vector < Bitmap_Image::Image > & image, std::vector < size_t > & x, std::vector < size_t > & y,
                      size_t & width, size_t & height )
    {
        std::vector < std::pair < size_t, size_t> > imageSize( image.size() );

        for( size_t i = 0; i < image.size(); ++i )
            imageSize[i] = std::pair < size_t, size_t >( image[i].width(), image[i].height() ) ;
                      ...
                      std::vector < size_t > & y, size_t & width, size_t & height )
    {
        ...
        for( std::vector < std::pair< size_t, size_t > >::const_iterator im = imageSize.begin();
             im != imageSize.end(); ++im ) {
            if( maximumWidth == 0 )
        ...
        width  = randomValue<size_t>( 1, maximumWidth  + 1 );
        height = randomValue<size_t>( 1, maximumHeight + 1 );

        x.resize( imageSize.size() );
        y.resize( imageSize.size() );

        for( size_t i = 0; i < imageSize.size(); ++i ) {
            x[i] = randomValue<size_t>( imageSize[i].first  - width );
            y[i] = randomValue<size_t>( imageSize[i].second - height );
        }
    }
    ...
    {
        return std::pair <size_t, size_t>( image.width(), image.height() );
    }

    size_t rowSize( size_t width, uint8_t colorCount, uint8_t alignment )
    {
        size_t size = width * colorCount;
        if( size % alignment != 0 )
