...
namespace Function_Pool
{
	...
	struct OutputInfo
	{
		...
		bool checkEqual()
		{
			...
			equality.empty(); // to guarantee that no one can use it second time
			return ok;
		}
	...
	protected:
		...
		void _task(size_t taskId)
		{
			...
			case _Invert:
				...
				break;
			case _IsEqual:
				_dataOut.equality[taskId] = Image_Function::IsEqual( _infoIn1->image, _infoIn1->startX[taskId], _infoIn1->startY[taskId],
											_infoIn2->image, _infoIn2->startX[taskId], _infoIn2->startY[taskId],
											_infoIn1->width[taskId], _infoIn1->height[taskId] );
				break;
			case _Maximum:
