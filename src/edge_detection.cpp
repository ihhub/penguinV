...
namespace
{
    ...
    void medianFilter(std::vector< int > & data, uint32_t filterSize)
    {
        if (data.empty())
        ...
        if (filterSize < 3u || (filterSize % 2 != 0))
            throw imageException("The size of a filter must be no smaller than 3 and uneven");
        ...
        for (size_t i = 0; i < size; ++i) {
            ...
            const size_t end = (i + halfFilterSize < size) ? (i + halfFilterSize) : size;
            const size_t width = end - start;
            const int * value = data.data() + start;

            for (size_t j = 0u; j < width; ++j, ++value)
                filter[j] = *(value);

            int * startSort = filter.data();
            int * medianSort = startSort + width / 2;
            int * endSort = startSort + width;

            std::nth_element(startSort, medianSort, endSort);
