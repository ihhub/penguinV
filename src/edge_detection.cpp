...
template<typename _Type>
void findEdgePoints(std::vector < _Type > & positive, std::vector < _Type > & negative, std::vector < int > & data, std::vector < int > & first, std::vector < int > & second, const EdgeParameter & edgeParameter, bool forwardDirection);

template<typename _Type>
void getEdgePoints(std::vector <_Type> & edge, const std::vector < int > & image, const std::vector < int > & first, const std::vector < int > & second, const EdgeParameter & edgeParameter);
...
void EdgeDetectionHelper::find(EdgeDetection<double> & edgeDetection, const PenguinV_Image::Image & image, uint32_t x, uint32_t y, uint32_t width, uint32_t height, const EdgeParameter & edgeParameter)
{
    ...
    if ( horizontalEdgeDetectionBase ) {
        ...
        for ( uint32_t rowId = 0u; rowId < height - edgeParameter.groupFactor + 1u; rowId += edgeParameter.skipFactor, imageY += (rowSize * edgeParameter.skipFactor) ) {
            ...
            if ( edgeParameter.direction == EdgeParameter::LEFT_TO_RIGHT ) {
                if ( edgeParameter.gradient == EdgeParameter::POSITIVE || edgeParameter.gradient == EdgeParameter::ANY )
                    createPositiveXEdge<double>( edgePositive, edgeDetection.positiveEdgePoint, x, y + rowId + (edgeParameter.groupFactor - 1) / 2.0f );

                if ( edgeParameter.gradient == EdgeParameter::NEGATIVE || edgeParameter.gradient == EdgeParameter::ANY )
                    createNegativeXEdge<double>( edgeNegative, edgeDetection.negativeEdgePoint, x + width - 1, y + rowId + (edgeParameter.groupFactor - 1) / 2.0f );
            }
            else {
                if ( edgeParameter.gradient == EdgeParameter::POSITIVE || edgeParameter.gradient == EdgeParameter::ANY )
                    createNegativeXEdge<double>( edgeNegative, edgeDetection.positiveEdgePoint, x + width - 1, y + rowId + (edgeParameter.groupFactor - 1) / 2.0f );

                if ( edgeParameter.gradient == EdgeParameter::NEGATIVE || edgeParameter.gradient == EdgeParameter::ANY )
                    createPositiveXEdge<double>( edgePositive, edgeDetection.negativeEdgePoint, x, y + rowId + (edgeParameter.groupFactor - 1) / 2.0f );
            }
        }
    }
    else {
        ...
        for ( uint32_t rowId = 0u; rowId < width - edgeParameter.groupFactor + 1u; rowId += edgeParameter.skipFactor, imageX += edgeParameter.skipFactor ) {
            ...
            if ( edgeParameter.direction == EdgeParameter::TOP_TO_BOTTOM ) {
                if ( edgeParameter.gradient == EdgeParameter::POSITIVE || edgeParameter.gradient == EdgeParameter::ANY )
                    createPositiveYEdge<double>( edgePositive, edgeDetection.positiveEdgePoint, x + rowId + (edgeParameter.groupFactor - 1) / 2.0f, y );

                if ( edgeParameter.gradient == EdgeParameter::NEGATIVE || edgeParameter.gradient == EdgeParameter::ANY )
                    createNegativeYEdge<double>( edgeNegative, edgeDetection.negativeEdgePoint, x + rowId + (edgeParameter.groupFactor - 1) / 2.0f, y + height - 1 );
            }
            else {
                if ( edgeParameter.gradient == EdgeParameter::POSITIVE || edgeParameter.gradient == EdgeParameter::ANY )
                    createNegativeYEdge<double>( edgeNegative, edgeDetection.positiveEdgePoint, x + rowId + (edgeParameter.groupFactor - 1) / 2.0f, y + height - 1 );

                if ( edgeParameter.gradient == EdgeParameter::NEGATIVE || edgeParameter.gradient == EdgeParameter::ANY )
                    createPositiveYEdge<double>( edgePositive, edgeDetection.negativeEdgePoint, x + rowId + (edgeParameter.groupFactor - 1) / 2.0f, y );
            }
        }
    }
}
...
void EdgeDetectionHelper::find(EdgeDetection<float> & edgeDetection, const PenguinV_Image::Image & image, uint32_t x, uint32_t y, uint32_t width, uint32_t height, const EdgeParameter & edgeParameter)
{
    Image_Function::VerifyGrayScaleImage(image);
    Image_Function::ParameterValidation(image, x, y, width, height);
    edgeParameter.verify();
    ...
    if (horizontalEdgeDetectionBase) {
        ...
        for (uint32_t rowId = 0u; rowId < height - edgeParameter.groupFactor + 1u; rowId += edgeParameter.skipFactor, imageY += (rowSize * edgeParameter.skipFactor)) {
            ...
            std::vector< float > edgePositive;
            std::vector< float > edgeNegative;
            findEdgePoints(edgePositive, edgeNegative, data, firstDerivative, secondDerivative, edgeParameter, (edgeParameter.direction == EdgeParameter::LEFT_TO_RIGHT));

            if (edgeParameter.direction == EdgeParameter::LEFT_TO_RIGHT) {
                if (edgeParameter.gradient == EdgeParameter::POSITIVE || edgeParameter.gradient == EdgeParameter::ANY)
                    createPositiveXEdge<float>(edgePositive, edgeDetection.positiveEdgePoint, x, y + rowId + (edgeParameter.groupFactor - 1) / 2.0f);

                if (edgeParameter.gradient == EdgeParameter::NEGATIVE || edgeParameter.gradient == EdgeParameter::ANY)
                    createNegativeXEdge<float>(edgeNegative, edgeDetection.negativeEdgePoint, x + width - 1, y + rowId + (edgeParameter.groupFactor - 1) / 2.0f);
            }
            else {
                if (edgeParameter.gradient == EdgeParameter::POSITIVE || edgeParameter.gradient == EdgeParameter::ANY)
                    createNegativeXEdge<float>(edgeNegative, edgeDetection.positiveEdgePoint, x + width - 1, y + rowId + (edgeParameter.groupFactor - 1) / 2.0f);

                if (edgeParameter.gradient == EdgeParameter::NEGATIVE || edgeParameter.gradient == EdgeParameter::ANY)
                    createPositiveXEdge<float>(edgePositive, edgeDetection.negativeEdgePoint, x, y + rowId + (edgeParameter.groupFactor - 1) / 2.0f);
            }
        }
    }
    else {
        ...
        for (uint32_t rowId = 0u; rowId < width - edgeParameter.groupFactor + 1u; rowId += edgeParameter.skipFactor, imageX += edgeParameter.skipFactor) {
            ...
            if (edgeParameter.direction == EdgeParameter::TOP_TO_BOTTOM) {
                if (edgeParameter.gradient == EdgeParameter::POSITIVE || edgeParameter.gradient == EdgeParameter::ANY)
                    createPositiveYEdge<float>(edgePositive, edgeDetection.positiveEdgePoint, x + rowId + (edgeParameter.groupFactor - 1) / 2.0f, y);

                if (edgeParameter.gradient == EdgeParameter::NEGATIVE || edgeParameter.gradient == EdgeParameter::ANY)
                    createNegativeYEdge<float>(edgeNegative, edgeDetection.negativeEdgePoint, x + rowId + (edgeParameter.groupFactor - 1) / 2.0f, y + height - 1);
            }
            else {
                if (edgeParameter.gradient == EdgeParameter::POSITIVE || edgeParameter.gradient == EdgeParameter::ANY)
                    createNegativeYEdge<float>(edgeNegative, edgeDetection.positiveEdgePoint, x + rowId + (edgeParameter.groupFactor - 1) / 2.0f, y + height - 1);

                if (edgeParameter.gradient == EdgeParameter::NEGATIVE || edgeParameter.gradient == EdgeParameter::ANY)
                    createPositiveYEdge<float>(edgePositive, edgeDetection.negativeEdgePoint, x + rowId + (edgeParameter.groupFactor - 1) / 2.0f, y);
            }
        }
    }
}
