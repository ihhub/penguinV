...
namespace Image_Function
{
    ...
    void ReplaceChannel( const Image & channel, uint32_t startXChannel, uint32_t startYChannel, Image & rgb, uint32_t startXRgb, uint32_t startYRgb, uint32_t width,
                         uint32_t height, uint8_t channelId )
    {
    ...
    void RgbToRgba( const Image & in, uint32_t startXIn, uint32_t startYIn, Image & out, uint32_t startXOut, uint32_t startYOut, uint32_t width, uint32_t height )
    {
        ValidateImageParameters( in, startXIn, startYIn, out, startXOut, startYOut, width, height );
        VerifyRGBImage( in );
        VerifyRGBAImage( out );
    ...
    void RgbaToRgb( const Image & in, uint32_t startXIn, uint32_t startYIn, Image & out, uint32_t startXOut, uint32_t startYOut, uint32_t width, uint32_t height )
    {
        ValidateImageParameters( in, startXIn, startYIn, out, startXOut, startYOut, width, height );
        VerifyRGBAImage( in );
        VerifyRGBImage( out );
    ...
    void Shift( const Image & in, uint32_t startXIn, uint32_t startYIn, Image & out, uint32_t startXOut, uint32_t startYOut, uint32_t width, uint32_t height,
                double shiftX, double shiftY )
    {
        ...
        const uint32_t emptyLeftArea
            = ( shiftXIntegral < 0 && startXIn < static_cast<uint32_t>( -shiftXIntegral ) ) ? ( static_cast<uint32_t>( -shiftXIntegral ) - startXIn ) : 0u;
        const uint32_t emptyRightArea = ( shiftXIntegral > 0 && limitX < startXIn + width + static_cast<uint32_t>( shiftXIntegral ) )
                                            ? ( startXIn + width + static_cast<uint32_t>( shiftXIntegral ) - limitX )
                                            : 0u;
        const uint32_t realWidth = width - emptyLeftArea - emptyRightArea;

        const uint32_t emptyTopArea
            = ( shiftYIntegral < 0 && startYIn < static_cast<uint32_t>( -shiftYIntegral ) ) ? ( static_cast<uint32_t>( -shiftYIntegral ) - startYIn ) : 0u;
        const uint32_t emptyBottomArea = ( shiftYIntegral > 0 && limitY < startYIn + height + static_cast<uint32_t>( shiftYIntegral ) )
                                             ? ( startYIn + height + static_cast<uint32_t>( shiftYIntegral ) - limitY )
                                             : 0u;
