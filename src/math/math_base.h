...
struct PointBase2D
{
    ...
    PointBase2D( const _Type _x, const _Type _y )
        : x( _x )
        , y( _y )
    {
        // Do nothing.
...
struct PointBase3D : public PointBase2D<_Type>
{
    ...
    PointBase3D( const _Type _x, const _Type _y, const _Type _z )
...
public:
    ...
    LineBase2D( const PointBase2D<_Type> & start, const PointBase2D<_Type> & end )
        : _position( start )
    {
        if ( start == end ) {
            ...
            const _Type xDiff = end.x - start.x;
            const _Type yDiff = end.y - start.y;
    ...
    // Angle is in radians
    LineBase2D( const PointBase2D<_Type> & position_, const _Type angle_ )
        : _position( position_ )
    ...
    {
        // It is based on Graphics Gems III, Faster Line Segment Intersection, p. 199-202
