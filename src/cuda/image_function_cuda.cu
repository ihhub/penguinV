...
namespace
{
    struct FunctionRegistrator
    {
        ...
        FunctionRegistrator()
        {
            ...
            table.Normalize = &Image_Function_Cuda::Normalize;
    ...
    __global__ void normalizeCuda( const uint8_t * in1, uint32_t rowSizeIn1, const uint8_t * in2, uint32_t rowSizeIn2, uint8_t * out, uint32_t rowSizeOut, uint32_t width,
                                   uint32_t height )
    {
    ...
    __global__ void setPixelCuda( uint8_t * in, uint32_t rowSize, uint32_t width, uint32_t height, uint32_t * pointX, uint32_t * pointY, uint32_t pointSize,
                                  uint32_t value )
    {
...
namespace Image_Function_Cuda
{
               ...
               bool horizontal, bool vertical )
    {
        ...
        if ( !horizontal && !vertical ) {
            ...
            const uint8_t * inY = in.data() + startYIn * rowSizeIn + startXIn * colorCount;
            uint8_t * outY = out.data() + startYOut * rowSizeOut + startXOut * colorCount;
            launchKernel2D( flipCuda, width, height, inY, rowSizeIn, outY, rowSizeOut, width, height, horizontal, vertical );
        }
    }
    ...
    bool IsEqual( const Image & in1, uint32_t startX1, uint32_t startY1, const Image & in2, uint32_t startX2, uint32_t startY2, uint32_t width, uint32_t height )
    {
        ...
        launchKernel2D( isEqualCuda, width, height, in1Y, rowSizeIn1, in2Y, rowSizeIn2, width, height, result.data() );
    ...
    void ProjectionProfile( const Image & image, uint32_t x, uint32_t y, uint32_t width, uint32_t height, bool horizontal, std::vector<uint32_t> & projection )
    {
        ...
        launchKernel2D( ( horizontal ? projectionProfileHorizontalCuda : projectionProfileVerticalCuda ), width, height, imageX, rowSize, width, height,
                        projectionCuda.data() );
    ...
    void SetPixel( Image & image, uint32_t x, uint32_t y, uint8_t value )
    {
        ...
        launchKernel1D( setPixelCuda, 1, image.data(), image.rowSize(), image.width(), image.height(), x, y, value );
    }
    ...
    void SetPixel( Image & image, const std::vector<uint32_t> & X, const std::vector<uint32_t> & Y, uint8_t value )
    {
        ...
        if ( X.size() > 0 ) {
            ...
            launchKernel1D( setPixelCuda, static_cast<uint32_t>( X.size() ), image.data(), image.rowSize(), width, height, pointX.data(), pointY.data(), pointX.size(),
                            value );
        }
    }
